...
function updateMember($dni, $name, $surname, $mail){
//Select active members
    //return: 
    //0-dni 1-name 2-surname 3-mail
    $conexion = ConnectDataBase();
    $query = "UPDATE members SET DNI='$dni', NAME='$name', SURNAME='$surname' WHERE mail= '$mail'";
    $result = mysqli_query($conexion, $query);
    if(!$result){
          die("Query error". mysqli_error($conexion));
...
function updatePicture($picture, $dni){

//Select active members
    //return: 
    //0-picture
    $conexion = ConnectDataBase();
    $query = "UPDATE members SET PICTURE='$picture' WHERE dni= '$dni'";
    $result = mysqli_query($conexion, $query);
    if(!$result){
          die("Query error". mysqli_error($conexion));
...
function searchMemberByDni($dni) {
    //Select one member
    //return: 
    //0-dni 1-name 2-surname 3-mail 4-password 5-account 6-admin
    $conexion = ConnectDataBase();
...
function searchMemberByMail($mailP) {
    //Select one member
    //return: 
    //0-dni 1-name 2-surname 3-mail 4-password 5-account 6-admin
    $conexion = ConnectDataBase();
    $query = "SELECT * FROM members WHERE mail='$mailP'";
